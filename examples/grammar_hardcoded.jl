...
terminals = ["ran", "saw", "Alice", "Bob", "and", "ERROR"]
...
code = @dice begin
    function S(max_depth)
        if max_depth == 0
            DistVector([DistEnum(terms_enum, "ERROR")])
        else
    ...
    function NP(max_depth)
        if max_depth == 0
            DistVector([DistEnum(terms_enum, "ERROR")])
        elseif flip(4/10)
            DistVector([DistEnum(terms_enum, "Alice")])
        elseif flip(4/6)
            DistVector([DistEnum(terms_enum, "Bob")])
        else
            prob_extend(
                prob_append(
                    NP(max_depth-1),
                    DistEnum(terms_enum, "and")),
                NP(max_depth-1)
            )
        end
    end
    ...
    function V(max_depth)
        if max_depth == 0
            DistVector([DistEnum(terms_enum, "ERROR")])
        elseif flip(4/10)
            DistVector([DistEnum(terms_enum, "ran")])
        else
            DistVector([DistEnum(terms_enum, "saw")])
        end
    end
