...
# Collect all terms so we can represent more them efficiently as a DistEnum
terms = Set()
...
terms_enum = DiceEnum(collect(terms))
...
code = @dice begin
    ...
    level = DistVector([DistEnum(terms_enum, start_term)])
    ...
    for _ in 1:num_steps
        ...
        for i in 1:length(level.contents)
            ...
            for (lhs, rhs_list) in rules
                ...
                cand_rhs = DistVector([DistEnum(terms_enum, t) for t in rhs_list[1][1]])
                for i in 2:length(rhs_list)
                    (rhs, p) = rhs_list[i]
                    take = flip(v[i])
                    cand_rhs = if take
                        DistVector([DistEnum(terms_enum, t) for t in rhs])
                    else
                        cand_rhs
                    end
                end
                ...
                lhs_match = prob_equals(DistEnum(terms_enum, lhs), level[i])
...
#==
5.618749 seconds (23.21 M allocations: 1.132 GiB, 3.02% gc time, 95.40% compilation time)
